 ...
 *                 foot switch.
  * Created: 2024-01-16
 * Author: erdiongson, 3:36PM
 * Version 3.71? (LNH) : i. This specific version should only be used for a customer
 *                         that wants a 2 minute delay on the SDB
 */
...
void Homing_Again_Manual(void) {
    ...
    //while (NUM > 0 || i_RUN_ZERO == 1) {
        ...
        __delay_ms(20000);
        __delay_ms(20000);
        __delay_ms(20000); //1 Minute Wait
        __delay_ms(20000);
        __delay_ms(20000);
        __delay_ms(20000); //2 Minutes Wait
#endif
//END        
        if (Stop == 1)
            break;
